...
#region variables
$script:ProductName = 'PSHW'
$script:Publisher = 'SLVStranger'
$script:ExtensionId = 'PSHW-Demo-Extension'
$script:ProductVersion = '1.0'
$script:ProductPath = ( '{0}\{1}-v{2}' -f $PSScriptRoot, $ProductName, $ProductVersion)
$script:ModuleName = 'PSHelloWorld'
$script:ModulePath = ('..\Extra\{0}\Output\{0}' -f $ModuleName)
#endregion
...
task ps_modules {
    ...
    if (-not (Test-Path ("$script:ProductPath\ps_modules\$script:ModuleName"))) {
        $null = New-Item -Path ("$script:ProductPath\ps_modules\$script:ModuleName") -ItemType Directory
    }
    Copy-Item -Path $script:ModulePath -Filter *.* -Recurse -Destination ("$script:ProductPath\ps_modules") -Force
}
#endregion
