...
$Endpoints += New-UDEndpoint -Url "/getgroup" -Method "POST" -Endpoint {
    param($DisplayName)
...
$Endpoints += New-UDEndpoint -Url "/newgroup" -Method "POST" -Endpoint {
    ...
    Try {
        ...
        New-AADGroup @params | ForEach-Object { [PSCustomObject]@{ 
                id              = $_.id; 
                createdDateTime = $_.createdDateTime
                description     = $_.description
                displayName     = $_.displayName
...
Invoke-RestMethod -Uri http://localhost:80/api/getgroup -Method POST -Body $body -OutVariable Result
#endregion
